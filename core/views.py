...
import json
from .utils import CATEGORIAS_JUEGOS
from core.models import Boleta, Cuenta, Producto, ProductoEnBoleta
...
def agregar_productos(request, id=None):
	if request.method == "POST":
		...
		descuento_sub = request.POST.get("campo_descuento_sub")
		...
		try:
			...
			for producto in data:
				Producto.objects.create(
					...
					descuento_subs = producto.get("porc_descuento_subs", 0),
					imagen = None
				)
	...
	if producto_id:
		try:
			...
			producto_data = {
				...
				"descuento_subs": producto.descuento_subs,
			}
